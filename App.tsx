...
import { ExpenseItem, Category, SavedRecord, SavedItem } from './types.ts';
...
const INITIAL_EXPENSES: ExpenseItem[] = [
  { id: '1', category: Category.FIXED, name: '手機費', amount: 699 },
  { id: '2', category: Category.FIXED, name: '房租', amount: 12000 },
  { id: '3', category: Category.FIXED, name: '保費', amount: 4583 },
  { id: '4', category: Category.FIXED, name: '定期定額投資', amount: 5000 },
  { id: '5', category: Category.LIVING, name: '餐費', amount: 9000 },
  { id: '6', category: Category.LIVING, name: '日常用品', amount: 2000 },
  { id: '7', category: Category.LIVING, name: '交通費', amount: 1500 },
  { id: '10', category: Category.SAVINGS, name: '預備金存款', amount: 5000 },
  { id: '11', category: Category.SAVINGS, name: '旅遊基金', amount: 2000 },
  { id: '14', category: Category.LONG_TERM_DEBT, name: '學貸', amount: 3500 },
];
...
const App: React.FC = () => {
  ...
  const handleConfirm = () => {
    // 取得所有有金額的項目
    const selectedItems: SavedItem[] = expenses
      .filter(e => e.amount > 0)
      .map(e => ({
        category: e.category,
        name: e.name || '未命名項目',
    ...
    <div className="min-h-screen py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-12">
      <header className="text-center">
        <div className="inline-block bg-pink-100 text-pink-700 px-4 py-1 rounded-full text-sm font-bold mb-4">
          Visual Budgeting Tool
        </div>
        <h1 className="text-4xl font-extrabold text-pink-700 mb-2 tracking-tight">
          預算分配計算器
        </h1>
        <p className="text-pink-600 font-medium opacity-80">讓每一分預算都有跡可循</p>
      </header>
